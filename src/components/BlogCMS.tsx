...
import { Plus, Edit2, Trash2, Save, ArrowLeft, BookOpen, CheckCircle, Lock, ChevronDown, ChevronUp } from 'lucide-react';
...
function generateSchemaTemplate(type: string, post: CMSPost): string {
  ...
  if (type === 'Article') {
    return JSON.stringify({
      ...base,
      headline: post.title || 'Título del artículo',
      description: post.excerpt || 'Descripción del artículo',
      ...
      publisher: { '@type': 'Organization', name: 'Colmedikal', url: 'https://colmedikal.com', logo: { '@type': 'ImageObject', url: 'https://colmedikal.com/logo.png' } },
      url,
      image: post.image_url || 'https://colmedikal.com/og-image.png',
      keywords: post.tags || '',
    }, null, 2);
  }
  if (type === 'MedicalWebPage') {
    return JSON.stringify({
      ...base,
      name: post.title || 'Título',
      description: post.excerpt || 'Descripción',
      url,
      specialty: 'Medicina General',
      lastReviewed: post.publish_date || new Date().toISOString().split('T')[0],
  ...
  if (type === 'MedicalOrganization') {
    return JSON.stringify({
      ...
      telephone: '+593-2-XXXXXX',
      medicalSpecialty: 'Medicina Prepagada',
    }, null, 2);
  }
    ...
    <div className="border border-slate-200 rounded-2xl overflow-hidden">
      ...
      {open && (
        <div className="p-4 space-y-3 bg-white">
          <p className="text-[10px] text-slate-500">El Schema JSON-LD se inyecta en el <code className="bg-slate-100 px-1 rounded">&lt;head&gt;</code> de este artículo para mejorar los rich snippets en Google.</p>
...
export default function BlogCMS() {
  ...
  const handleSave = async () => {
    ...
    if (editing.schema_json.trim()) {
      try { JSON.parse(editing.schema_json); } catch { alert('El JSON del Schema no es válido. Corrígelo antes de guardar.'); return; }
    }
    setSaving(true);
    try {
      ...
      <div className="space-y-5 animate-in fade-in duration-200 max-w-3xl" data-schema-post>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-5">
          ...
          <div className="space-y-1">
            ...
            <textarea rows={16} value={editing.content}
